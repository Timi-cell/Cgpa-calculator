...
// document.getElementById("name").innerHTML = `Welcome, ${user.toUpperCase()}`;
// let courses = [];
// let units = [];
// let grades = [];
...
document.getElementById("submit").addEventListener("click", () => {
  ...
  if (courseName !== "" && courseUnits !== "" && gradeScores !== "") {
    ...
    alert("pls complete all the fields");
  }
});
...
document.getElementById("calc").addEventListener("click", () => {
  ...
  if (unitsFirst.length !== 0 && gradesFirst.length !== 0) {
    ...
    alert("you have no data inputted yet!");
  }
});
...
document.getElementById("cgpa").addEventListener("click", () => {
  ...
  if (!isNaN(firstGpa) && !isNaN(secondGpa)) {
    document.querySelector(
      ".cgpa"
    ).textContent = `Your CGPA for this session is ${
      (firstGpa + secondGpa) / 2
    }`;
  } else {
    alert("You need to calculate your GPA's first");
  }
});
